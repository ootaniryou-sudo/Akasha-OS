 ...
 *   standard / deep(+refactor) / committee(全 topK=2) / coarse(2 分割) / llm
 */
export class PlanGenerator {
  ...
  private async coarseVariant(task: Task): Promise<Decomposition> {
    ...
    return this.rule.decompose(task);
  }
  ...
  async expand(plan: Decomposition, subtask: Subtask): Promise<Decomposition> {
    const children: Subtask[] = [
      { id: `${subtask.id}-a`, parentId: subtask.parentId, order: subtask.order, role: `${subtask.role}:step1`, capability: subtask.capability, prompt: `Part 1 — ${subtask.prompt}` },
      { id: `${subtask.id}-b`, parentId: subtask.parentId, order: subtask.order + 1, role: `${subtask.role}:step2`, capability: subtask.capability, prompt: `Part 2 — ${subtask.prompt}`, ...(subtask.expertPolicy ? { expertPolicy: subtask.expertPolicy } : {}) },
    ];
