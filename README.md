...
>
> 数万台の型落ちスマートフォンと安価な有線ネットワークを結集し、1つの超巨大な仮想6.7T級の群知能（Collective Intelligence）を創発させる、超分散型エッジAIオーケストレーションオペレーティングシステム。
...
├── tsconfig.json             # TypeScript 設定（Node.js）
├── tsconfig.edge.json        # TypeScript 設定（ブラウザ Edge）
├── PROTOCOL.md               # バイナリプロトコル仕様書
├── README.md                 # ← このファイル
├── public/
│   ├── client.html           # ブラウザエッジクライアント
│   ├── node.html             # Node.js エッジクライアント
│   └── worker-inference.js   # WebWorker 推論スクリプト（esbuild 出力）
└── src/
    ├── index.ts              # 公開API エントリポイント
    ├── binary/
    │   ├── codec.ts          # 48バイトヘッダ エンコード/デコード
    │   └── protocol.ts       # 定数・型定義
    ├── bootstrap/
    │   └── akasha-bootstrapper.ts  # 起動・自動認識エンジン
    ├── client/
    │   ├── node-client.ts    # Node.js エッジクライアント
    │   └── webgpu-expert.ts  # WebGPU 専門家モデル実行
    ├── core/
    │   ├── orchestrator.ts   # マスターオーケストレーター
    │   └── inference-loop.ts # 推論メインループ
    ├── edge/
    │   └── worker-inference.ts # ブラウザWebWorker 推論
    ├── fault/
    │   └── fault-tolerance.ts  # フォールトトレランスエンジン
    ├── ipc/
    │   └── ring-buffer.ts    # ロックフリー SPSC リングバッファ
    ├── pool/
    │   └── object-pool.ts    # オブジェクトプール / バッファプール
    ├── plugin/
    │   ├── index.ts          # プラグイン公開API / Plugin barrel export
    │   ├── types.ts          # AkashaExpertPlugin インターフェース / Plugin interface
    │   └── registry.ts       # ホットプラグ対応レジストリ / Hot-plug registry
    ├── sim/
    │   ├── demo.ts           # デモシミュレーション
    │   └── selftest.ts       # 自己診断テスト
    ├── structures/
    │   ├── doubly-linked-list.ts  # 侵入的双方向リンクドリスト
    │   └── idle-cluster-pool.ts   # O(1) クラスタアイドルプール
    └── workers/
        ├── network-worker.ts # ネットワークワーカー（WebSocket I/O）
        └── router-worker.ts  # ルーターワーカー（セマンティックルーティング）
```
