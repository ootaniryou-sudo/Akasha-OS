...
import { HEADER_SIZE, MAX_PACKET_BYTES, clusterName, nowUs, } from '../binary/protocol.js';
...
const fault = new FaultToleranceEngine(nodePool, txPool, {
    ...
    shadowClusterId: 99 /* ClusterId.SHADOW_POOL */,
});
...
function sendToSlot(socketSlot, packetBuf, packetLen) {
    const frame = bufPool.acquire();
    const u8 = new Uint8Array(frame);
    u8[0] = 2 /* IpcKind.OUTBOUND_PACKET */;
...
function dispatchCompute(tx, node, activation, flags) {
    const expectedUs = nodePool.deadlineUs(node.nodeId, data.marginUs);
    const buf = bufPool.acquire();
    const header = buildComputeHeader(tx.txId, node.nodeId, tx.clusterId, nowUs(), expectedUs, tx.seq, flags);
    const len = BinaryCodec.encode(buf, { ...header, payload: activation });
    sendToSlot(node.socketSlot, buf, len);
    bufPool.release(buf);
}
function cacheActivation(txId, activation) {
...
function handleRegister(header, socketSlot) {
    const shadow = header.flags !== 0 && header.seq !== 0 ? BigInt(header.seq) : 0n; // seq may carry shadow hint
...
function handleResult(header, buf) {
...
function handleInboundPacket(frame, len) {
...
function handleSubmitPrompt(frame, len) {
    ...
    const activation = new Float32Array(frame.buffer, frame.byteOffset + actOffset, floatCount);
    ...
    dispatchCompute(tx, node, owned, 0 /* Flag.NONE */);
    parentPort?.postMessage({
        type: 'dispatch',
        txId: txId.toString(),
...
function onFaultTimeout(tx, shadow) {
    const act = getCachedActivation(tx.txId);
    if (!act)
        return;
    dispatchCompute(tx, shadow, act, 1 /* Flag.SHADOW */);
    parentPort?.postMessage({
        type: 'failover',
...
function drainInbound() {
    for (;;) {
        ...
        if (n < 0)
            break;
        const kind = scratchIn[0];
        if (kind === 1 /* IpcKind.INBOUND_PACKET */)
            handleInboundPacket(scratchIn, n);
        else if (kind === 4 /* IpcKind.NODE_GONE */)
            handleNodeGone(scratchIn);
    }
}
function drainControl() {
    for (;;) {
        ...
        if (n < 0)
            break;
        const kind = scratchCtrl[0];
        if (kind === 6 /* IpcKind.SUBMIT_PROMPT */)
            handleSubmitPrompt(scratchCtrl, n);
    }
}
function loop() {
    drainInbound();
...
setInterval(() => {
    parentPort?.postMessage({
        ...
        idleGeneral: nodePool.idleCount(1 /* ClusterId.GENERAL */),
        idleMath: nodePool.idleCount(2 /* ClusterId.MATH */),
        idleShadow: nodePool.idleCount(99 /* ClusterId.SHADOW_POOL */),
    });
}, 5_000).unref();
