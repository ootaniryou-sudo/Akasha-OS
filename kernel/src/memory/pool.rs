...
impl TensorPool {
    ...
    pub fn new(hidden_size: usize, max_slots: usize, scratch_size: usize) -> Self {
        ...
        // SAFETY: zero-initialised float buffer; all-zero is a valid f32 (0.0).
    ...
    pub fn acquire(&self) -> Option<usize> {
        loop {
            ...
            if self
                ...
                .is_ok()
            {
                // Zero the slot (cheap: L1-cache-hot write)
                let start = slot * self.hidden_size;
                self.memory[start..start + self.hidden_size].fill(0.0);
                return Some(slot);
            }
    ...
    ///
    /// The caller writes tensor data directly into this buffer —
    /// no copy, no allocation.
    pub fn slot_mut(&self, slot: usize) -> &mut [f32] {
        assert!(slot < self.max_slots);
        let start = slot * self.hidden_size;
        // SAFETY: `slot` is bounds-checked; each slot is disjoint.
        // We return a mutable borrow — the caller must ensure no aliasing.
        unsafe {
            let ptr = self.memory.as_ptr().add(start) as *mut f32;
            std::slice::from_raw_parts_mut(ptr, self.hidden_size)
        }
    }
    ...
    pub fn slot(&self, slot: usize) -> &[f32] {
        assert!(slot < self.max_slots);
        let start = slot * self.hidden_size;
        &self.memory[start..start + self.hidden_size]
    }
    ...
    pub fn memory_bytes(&self) -> usize {
        (self.hidden_size * self.max_slots + self.scratch_size) * 4
    }
}

// SAFETY: TensorPool uses atomic operations for slot management.
//          The inner `Box<[f32]>` is never exposed mutably to multiple
//          threads simultaneously (slot_mut borrows are caller-managed).
