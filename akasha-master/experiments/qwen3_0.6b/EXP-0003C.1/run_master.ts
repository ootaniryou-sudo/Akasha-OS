 ...
 *
 * Learning Depth Hypothesis:
 *   学習対象が深いほど Sample Complexity が増える (0002E.3 → 0003A → 0003C)
 *   → Contextual Bandit (UCB) は Q-Learning より少ないサンプルで学習できるか?
 *
 * 定式化 (ArcAsha は「文脈を見て選び、即座に報酬を得る」構造):
 *   Context (state) → Arm (node) → Reward (Quality + Latency + Cost + Stability)
 *
 * 手法比較:
 *   ① Fixed          : 手設計の重み (ベースライン, 探索なし)
 *   ② Q-Learning     : ε-greedy テーブル (0003C と同じ)
 *   ③ UCB1           : Q̄_a + √(2 ln t / n_a) — 楽観的探索 (サンプル効率)
 *   ④ Thompson       : Beta 事後分布からサンプリング (ベイズ探索)
 *
 * サンプルスケーリング: Cumulative Regret を 24 / 50 / 100 / 150 ステップで測定。
 * 仮説: UCB / Thompson は Q-Learning より少ないサンプルで Fixed を超える。
 *
 * Usage:
...
const TOTAL_STEPS = 60;           // サンプルスケーリングの上限
const CHECKPOINTS = [24, 50, 60];
const STEPS_PER_PHASE = 20;       // フェーズ切り替え (3 フェーズ × 20 = 60)

const ETA = 0.3;
const EPSILON = 0.15;
const UCB_C = 2.0;                // UCB 探索係数

// フェーズ定義: 注入を交互に (baseline / latency / capjump)
...
async function main() {
  ...
  console.log('EXP-0003C.1 — Contextual Bandit (UCB) Router');
  console.log('═'.repeat(60));
  console.log('  Context (state) → Arm (node) → Reward');
  console.log('  Methods: Fixed | Q-Learning | UCB1 | Thompson');
  console.log(`  Sample scaling: ${CHECKPOINTS.join(' → ')} steps`);
  console.log('  Hypothesis: UCB/Thompson beat Fixed with fewer samples than Q-Learning\n');
  ...
  wss.on('connection', (ws: WebSocket, req) => {
    ...
    ws.on('message', (raw: Buffer) => {
      ...
      if (msg.type === 'register') {
        ...
        ws.send(JSON.stringify({ type: 'register_ack', node_id: nodeId, master: 'EXP-0003C.1' }));
  ...
  async function runExperiment() {
    console.log('\n═'.repeat(60));
    console.log('EXPERIMENT — Contextual Bandit: sample scaling');
    console.log('═'.repeat(60));
    ...
    // ── Results ────────────────────────────────────────────────────────────
    console.log('\n═'.repeat(60));
    console.log('RESULTS — Contextual Bandit: sample scaling');
    console.log('═'.repeat(60));

    console.log('\n  Cumulative Regret at checkpoints (samples → cumRegret):');
    console.log('  ┌─────────┬─────────┬──────────┬──────────┬──────────┐');
    ...
    for (const method of ['qlearn', 'ucb', 'thompson'] as const) {
      ...
      console.log(`    ${method.padEnd(10)}: ${crossed > 0 ? `crossed at ${crossed} samples ✅` : 'not crossed within 150 samples ❌'}`);
    }

    // Learning Depth 比較
    console.log('\n  Learning Depth Hypothesis (sample to beat Fixed):');
    ...
    console.log('    Bandit (0003C.1)  : see above');

    const outDir = path.resolve('experiments/qwen3_0.6b/EXP-0003C.1/output');
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, 'summary.json'), JSON.stringify({
      experiment: 'EXP-0003C.1',
      description: 'Contextual Bandit (UCB/Thompson) Router — sample scaling',
      timestamp: new Date().toISOString(),
      config: {
        total_steps: TOTAL_STEPS, checkpoints: CHECKPOINTS, phases: PHASES.map(p => p.name),
        eta: ETA, epsilon: EPSILON, ucb_c: UCB_C,
