...
import { Cmd, ClusterId, HEADER_SIZE, MAGIC, nowUs } from '../binary/protocol.js';
...
assert(h.command === Cmd.COMPUTE_TASK, 'cmd');
assert(h.txId === 42n, 'tx');
assert(h.clusterId === ClusterId.MATH, 'cluster');
...
pool.register(1n, ClusterId.MATH, 1);
pool.register(2n, ClusterId.MATH, 2);
pool.register(3n, ClusterId.GENERAL, 3);
const a = pool.acquireIdle(ClusterId.MATH);
assert(a && a.nodeId === 1n && a.status === STATUS_COMPUTING, 'FIFO idle');
const b = pool.acquireIdle(ClusterId.MATH);
assert(b && b.nodeId === 2n, 'second idle');
assert(pool.acquireIdle(ClusterId.MATH) === null, 'exhausted');
pool.releaseToIdle(1n);
const c = pool.acquireIdle(ClusterId.MATH);
...
assert(routeCluster('Solve 256 * 4') === ClusterId.MATH, 'route math');
assert(routeCluster('hello world') === ClusterId.GENERAL, 'route general');
assert(routeCluster('write a function please') === ClusterId.CODE, 'route code');
