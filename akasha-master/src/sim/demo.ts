...
async function main(): Promise<void> {
  const orch = new AkashaOrchestrator({
    ...
    onEvent: (ev) => {
      switch (ev.type) {
        ...
        case 'result':
          console.log(
            `✅ [${ev.txId}] by ${ev.nodeId} in ${ev.latencyUs}μs | [${ev.sample.map((x) => x.toFixed(4)).join(', ')}...]${ev.failover ? ' [SHADOW]' : ''}`,
          );
          break;
        case 'queue':
        ...
        case 'stats':
          console.log(
            `📊 mesh nodes=${ev.nodes} inFlight=${ev.inFlight} idle G/M/S=${ev.idleGeneral}/${ev.idleMath}/${ev.idleShadow}`,
          );
          break;
        case 'error':
  ...
  const WS = WebSocket as unknown as import('../client/node-client.js').WsConstructor;
  ...
  const mathShadow = new AkashaEdgeNode({
    url,
    nodeId: 1099n,
    clusterId: ClusterId.SHADOW_POOL,
    simulateLatencyMs: 3,
    WebSocketImpl: WS,
  });
